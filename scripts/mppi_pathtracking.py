...
class MPPIControllerForPathTracking():
    ...
    def calc_control_input(self, observed_x: np.ndarray) -> Tuple[float, np.ndarray]:
        ...
        # clear sampled trajectories if visualization disabled
    ...
    def _c(self, x_t: np.ndarray) -> float:
        ...
        return (self.stage_cost_weight[0]*(x-ref_x)**2 + self.stage_cost_weight[1]*(y-ref_y)**2
                + self.stage_cost_weight[2]*(yaw_diff)**2 + self.stage_cost_weight[3]*(v-ref_v)**2)

    def _c_vec(self, x_t: np.ndarray) -> np.ndarray:
        """stage cost for K samples at once. x_t shape: (K, 4) -> (K,)"""
        x = x_t[:, 0]
        y = x_t[:, 1]
        yaw = x_t[:, 2]
        v = x_t[:, 3]
        ...
        yaw_diff = np.arctan2(np.sin(yaw-ref_yaw), np.cos(yaw-ref_yaw))
        return (self.stage_cost_weight[0]*(x-ref_x)**2
    ...
    def _phi(self, x_T: np.ndarray) -> float:
        ...
        _, ref_x, ref_y, ref_yaw, ref_v = self._get_nearest_waypoint(x, y)
        yaw_diff = np.arctan2(np.sin(yaw-ref_yaw), np.cos(yaw-ref_yaw))
        return (self.terminal_cost_weight[0]*(x-ref_x)**2 + self.terminal_cost_weight[1]*(y-ref_y)**2
                + self.terminal_cost_weight[2]*(yaw_diff)**2 + self.terminal_cost_weight[3]*(v-ref_v)**2)

    def _phi_vec(self, x_T: np.ndarray) -> np.ndarray:
        """terminal cost for K samples at once. x_T shape: (K, 4) -> (K,)"""
        x = x_T[:, 0]
        y = x_T[:, 1]
        yaw = x_T[:, 2]
        v = x_T[:, 3]
        ...
        _, ref_x, ref_y, ref_yaw, ref_v = self._get_nearest_waypoints_vec(x, y)
        yaw_diff = np.arctan2(np.sin(yaw-ref_yaw), np.cos(yaw-ref_yaw))
        return (self.terminal_cost_weight[0]*(x-ref_x)**2
    ...
    def _get_nearest_waypoints_vec(self, x: np.ndarray, y: np.ndarray):
        ...
        SEARCH_IDX_LEN = 200
        prev_idx = self.prev_waypoints_idx
        search_slice = self.ref_path[prev_idx:prev_idx + SEARCH_IDX_LEN]
        dx = x[:, np.newaxis] - search_slice[np.newaxis, :, 0]
        dy = y[:, np.newaxis] - search_slice[np.newaxis, :, 1]
        d_sq = dx**2 + dy**2
        rel_idx = np.argmin(d_sq, axis=1)
        nearest_idx = rel_idx + prev_idx
        return (nearest_idx,
                search_slice[rel_idx, 0],
                search_slice[rel_idx, 1],
                search_slice[rel_idx, 2],
                search_slice[rel_idx, 3])
    ...
    def _F_vec(self, x_t: np.ndarray, v_t: np.ndarray) -> np.ndarray:
        """next state for K samples at once. x_t: (K, 4), v_t: (K, 2) -> (K, 4)"""
        x = x_t[:, 0]
        y = x_t[:, 1]
        yaw = x_t[:, 2]
        v = x_t[:, 3]
...
def run_simulation_mppi_pathtracking() -> None:
    ...
    vehicle.reset(
        init_state = np.array([0.0, 1.0, 0.0, 0.0]),
    )
    ...
    vehicle.save_animation("mppi_pathtracking_demo.mp4", interval=int(delta_t * 1000), movie_writer="ffmpeg")
